<x-app-layout>
    <div class="max-w-4xl mx-auto p-6 lg:p-8">
        <h1 class="text-2xl font-semibold text-gray-900 mb-6">Create New Event</h1>
            ...
            <form action="{{ route('admin.events.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="datetime-local" name="event_date_at" id="event_date_at" value="{{ old('event_date_at') }}" required
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    </div>
                    <div>
                        <label for="sale_start_at" class="block font-medium text-gray-700">Sale Start</label>
                        <input type="datetime-local" name="sale_start_at" id="sale_start_at" value="{{ old('sale_start_at') }}" required
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    </div>
                    <div>
                        <label for="sale_end_at" class="block font-medium text-gray-700">Sale End</label>
                        <input type="datetime-local" name="sale_end_at" id="sale_end_at" value="{{ old('sale_end_at') }}" required
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="max_number_allowed" id="max_number_allowed" value="{{ old('max_number_allowed', 5) }}" required min="1"
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    </div>

                    <div>
                        <label for="cover_image" class="block font-medium text-gray-700">Cover Image (Optional, maximum 2 mb)</label>
                ...
                <div class="flex items-center">
                    <input type="checkbox" name="is_dynamic_price" id="is_dynamic_price" value="1" {{ old('is_dynamic_price') ? 'checked' : '' }}
                           class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                    <label for="is_dynamic_price" class="ml-2 block text-gray-900">
                        Enable Dynamic Pricing (Price increases as event nears)
                    </label>
                </div>
